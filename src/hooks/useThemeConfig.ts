...
export const useThemeConfig = () => {
    const [scheme] = useLocalStorage<ColorScheme>({key: 'mantine-color-scheme'});
    const [themeConfig, setThemeConfig] = useState<allThemeConfig>({} as allThemeConfig);

    const isDrakMode = scheme === 'dark';

    useEffect(() => {
        setThemeConfig({colors: {
